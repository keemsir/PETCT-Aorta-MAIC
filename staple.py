...
def getUniformScanXYZVals_standardalone(rtstStruct):
    ...
    yOffset = 0
    firstZValue = 0
    grid2Units = 4.07283  # 0.9765625
    grid1Units = 4.07283  # 0.9765625
    sliceThickness = 3.0
...
# folderListFull = subdirs(mainPath, join = True)
patientNumber = len(folderList)
...
for iterPatient in inList: # PatientKey
    ...
    for ModelList in folderList:
        ...
        patientStack_aorta = np.array(nib.load(os.path.join(mainPath, ModelList, iterPatient)).dataobj)
    ...
    # aortaSum.append(np.sum(mask2[:]))

    nii_stp = nib.Nifti1Image(mask2, affine=np.eye(4))
    nib.save(nii_stp, os.path.join(savePath, '{}.nii.gz'.format(iterPatient[:])))
