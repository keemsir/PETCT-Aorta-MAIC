...
cur_path = os.getcwd()
model_path = os.path.join(cur_path, 'sample/files')
model1 = os.path.join(model_path, '23010017_20141226.hdf5.h5')
model2 = os.path.join(model_path, '23010018_20141226.hdf5.h5')
model3 = os.path.join(model_path, '23010019_20141224.hdf5.h5')

f = h5py.File(model1, 'r')

ctarr = np.asarray(f['CT'])
petarr = np.asarray(f['PET'])
aorarr = np.asarray(f['Aorta'])
sizearr = np.asarray(f['Size'])

f.close()



print(petarr.shape, ctarr.shape, aorarr.shape, sizearr)
print(np.unique(petarr), np.unique(ctarr), np.unique(aorarr), np.unique(sizearr))
